package ru.d5kostya.skinswitcher.client;
...
import ru.d5kostya.skinswitcher.client.gui.screen.SkinCustomScreen;
import ru.d5kostya.skinswitcher.client.gui.style.VanillaLikeStyle;
...
import static ru.d5kostya.skinswitcher.client.api.SkinOption.SkinType.NICKNAME;
import static ru.d5kostya.skinswitcher.client.api.SkinOption.SkinType.URL;

public class skin_switcher implements ClientModInitializer {
    public static Config pathConfig = new Config("config/skin_switcher/path.config.json");
    public static Config config = new Config(getPath()+"/config.json");
    public static AlinLogger logger = new AlinLogger("skin_switcher");
    ...
    public static String getPath(){
        String path = pathConfig.getBoolean("USE_GLOBAL", false) && !FabricLoader.getInstance().isDevelopmentEnvironment()? (
                System.getProperty("os.name").toLowerCase(Locale.ROOT).contains("win") ?
                        pathConfig.getString("PATH", "{HOME}/skin_switcher") :
                        pathConfig.getString("PATH.UNIX", "/home/${USER}/skin_switcher")
        ) : "config/skin_switcher";
    ...
    public void onInitializeClient() {
        ...
        ClientLifecycleEvents.CLIENT_FULL_STARTED.register((s) -> {
            ...
            if(FabricLoader.getInstance().isDevelopmentEnvironment()){
                new ToastBuilder().setTitle(Component.translatable("skin_switcher"))
                        .setMessage(Component.translatable("skin_switcher.warning.test_mode")).setIcon(WARNING)
        ...
        if (container == null) {
            System.err.println("Mod container not found for id 'skin_switcher'");
            return;
        }

        ResourceManagerHelper.registerBuiltinResourcePack(packId, container, ResourcePackActivationType.NORMAL);
        System.out.println("Registered built-in resource pack: " + packId);
    }
        ...
        if(Player.isLicenseAccount()){
            ...
            if(!GsonHelper.jsonElementIsNull("capes", jsonObject)){
                for(JsonElement element : jsonObject.getAsJsonArray("capes")){
                    ...
                    ResourceLocation location = GuiUtils.getResourceLocation("skin_switcher", "cape_"+id.toLowerCase().replace("-", "_"));
                    ...
                    skin_switcher.cacheResourceLocations.put(url, location);
                    capes.put(id, location);
                }
            }
        } else{
            if(FabricLoader.getInstance().isDevelopmentEnvironment()){
                capes.put("test", GuiUtils.getResourceLocation("skin_switcher", "cape/test.png"));
