...
import ru.d5kostya.skinswitcher.client.gui.components.CapeButton;
import ru.d5kostya.skinswitcher.client.gui.components.NonCapeButton;
...
public class SelectCape extends Screen {
    ...
    public SelectCape(Screen parent, SkinOption skinOption, String skinKey) {
        super(Component.translatable("skin_switcher.select.cape"));
        this.parent = parent;
    ...
    protected void init() {
        ...
        for (String name : skin_switcher.capes.keySet()) {
            widgets_pages.add(new CapeButton(-widthComponent, 35 + font.lineHeight, widthComponent, heightComponent, skin_switcher.capes.get(name), skinOption, name, skinKey, skin_switcher.capesAliases.get(name)));
        }
    ...
    public void onClose() {
        ...
        try {
            skinOption.save();
            if(skin_switcher.currentSkin == skinOptionOriginal) skin_switcher.currentSkin = skinOption;
            skin_switcher.skinOptions.put(skinKey, skinOption);
