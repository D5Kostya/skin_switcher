package ru.d5kostya.skinswitcher.client.gui.components;
...
public class PlayerButton extends AbstractButton {
    ...
    public PlayerButton(int x, int y, int size, boolean showItem, boolean rotate, boolean autoRotate, boolean followMouse) {
        super(x, y, size, size * 2 + 20, Component.translatable("skin_switcher.open_gui"));
        this.size = size;
        this.rotate = rotate;
    ...
    public void renderWidget(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTicks) {
        ...
        try {
            GuiEntityRenderer.drawModel(
                    ...
                    size, rotation, followX, followY, skin_switcher.currentSkin == null ? skin_switcher.safeSkinOption : skin_switcher.currentSkin, partialTicks
            );
        } catch (Exception ignored){}
        guiGraphics.pose().popPose();
        AbstractStyle style = skin_switcher.config.getBoolean("MENU.ALINLIB", false) ? GuiUtils.getSelected() : skin_switcher.vanillaLikeStyle;
    ...
    @Override
    public void onPress() {
        AlinLib.MINECRAFT.setScreen(new SelectSkinPreset(AlinLib.MINECRAFT.screen));
    }
